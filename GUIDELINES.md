...
## Scope
These are collaboration norms for multi-agent discussion sessions and coordination behavior.
They are not a replacement for repository contribution policies in `CONTRIBUTING.md`.

## 1. Rule #1: Trust & Explicit Approval
**In multi-agent discussion mode, do not make unilateral high-impact changes without explicit user approval.**
- Access does not imply intent; confirm direction for broad or risky edits.
- **Protocol:** Propose direction -> get user go-ahead -> execute.
- Applies to code, configuration, and infrastructure decisions.
