...
- At least one agent CLI installed and in your `$PATH`:
  ...
  - `kimi` — [Kimi Code CLI](https://github.com/MoonshotAI/kimi-cli)
...
```bash
./start.sh --backend python
```

This starts both the Python backend (port 8421) and the frontend dev server (port 5174) together. Open **http://localhost:5174** in your browser.

To customize which agents participate:

```bash
./start.sh --backend python --agents claude,codex    # Only Claude and Codex
./start.sh --backend python --port 9000              # Different backend port
```

## 3. Create a session

When the app loads, you'll see the **session picker** — a list of previous sessions (if any) and a **NEW SESSION** button.

![Session picker](./images/session-picker.png)

Click **NEW SESSION**. You'll be prompted to:
...
![Create session](./images/roster-editor.png)
...
![Chat layout](./images/chat-layout.png)
