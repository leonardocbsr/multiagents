...
def _format_card_system_message(data: dict) -> str | None:
    ...
    if event_type == "card_phase_started" and isinstance(card, dict):
        return (
            f"Task phase started: [{card.get('id', '?')}] {card.get('title', '(untitled)')} "
            f"assigned to {data.get('agent', 'unknown')}"
        )
    if event_type == "card_phase_completed" and isinstance(card, dict):
        status = card.get("status")
        suffix = f" -> {status}" if status else ""
        return (
            f"Task phase completed: [{card.get('id', '?')}] {card.get('title', '(untitled)')} "
            f"by {data.get('agent', 'unknown')}{suffix}"
        )
    return None
...
class RoundMetrics:
    ...
    first_chunk_ms: dict[str, float] = field(default_factory=dict)
...
class SessionRunner:
    def __init__(
        ...
        self._round_metrics: dict[str, dict[int, RoundMetrics]] = {}
    ...
    async def broadcast(self, session_id: str, data: dict) -> int:
        ...
        for ws, result in zip(snapshot, results):
            if isinstance(result, Exception):
                ...
                session_metrics = self._round_metrics.get(session_id, {})
                target_round = data.get("round")
                metrics: RoundMetrics | None = None
                if isinstance(target_round, int):
                    metrics = session_metrics.get(target_round)
                elif session_metrics:
                    latest_round = max(session_metrics)
                    metrics = session_metrics.get(latest_round)
                if metrics:
    ...
    def cleanup_session(self, session_id: str, cancel_card_phase_tasks: bool = True) -> None:
        ...
        # Clean up agent pool — shut down persistent processes
        pool = self._agent_pools.pop(session_id, {})
        shutdown_coros = []
        for agent in pool.values():
            if agent._persistent:
                shutdown_coros.append(agent.shutdown_persistent())
            agent.cleanup()
        if shutdown_coros:
            async def _shutdown_all() -> None:
                await asyncio.gather(*shutdown_coros, return_exceptions=True)
            asyncio.ensure_future(_shutdown_all())
    ...
    async def start_card(self, session_id: str, card_id: str, agent_names: list[str] | list[dict]) -> None:
        ...
        agent_name = await self._resolve_card_agent_with_fallback(session_id, card, agent_names)
        if not agent_name:
    ...
    async def _execute(self, session_id: str, prompt: str, agent_names: list[str] | list[dict], start_round: int = 0) -> None:
        ...
        try:
            event_source = room.run_persistent(start_round=start_round)
            async for event in event_source:
                if isinstance(event, RoundStarted):
                    ...
                    self._round_metrics.setdefault(session_id, {})[round_number] = RoundMetrics(
                        ...
                        if engine:
                            try:
                                ...
                                if next_prompt:
                                    ...
                                    next_agent = await self._resolve_card_agent_with_fallback(
                                        session_id, card, agent_names,
                                    )
                                    if next_agent:
                    ...
                    await self._store_call(
                        self.store.append_agent_stream,
                        session_id,
                        event.agent_name,
                        event.round_number,
                        event.text,
                    ...
                    metrics = self._round_metrics.get(session_id, {}).get(event.round_number)
                    if metrics:
                        if metrics.stream_chunks.get(event.agent_name, 0) == 0:
                            metrics.first_chunk_ms[event.agent_name] = (
                    ...
                    session_metrics = self._round_metrics.get(session_id, {})
                    metrics = session_metrics.pop(event.round_number, None)
                    ...
                    if metrics:
                        ...
                        self._log_metric(
                            ...
                            first_chunk_ms={k: round(v, 2) for k, v in metrics.first_chunk_ms.items()},
                            agent_latency_ms=metrics.latencies_ms,
                            send_failures=metrics.send_failures,
                        )
                    if recorder:
                        recorder.record_round_ended(event.round_number, event.all_passed)
    ...
    async def _resolve_card_agent_with_fallback(
        self,
        session_id: str,
        card: Card,
        agent_names: list[str] | list[dict],
    ) -> str | None:
        resolved = self._resolve_card_agent(card)
        if resolved:
            return resolved

        candidates = _extract_agent_names(agent_names)
        if not candidates:
            return None
        fallback = candidates[0]

        changed = False
        if card.status == CardStatus.COORDINATING and not card.coordinator:
            card.coordinator = fallback
            changed = True
        elif card.status == CardStatus.PLANNING and not card.planner:
            card.planner = fallback
            changed = True
        elif card.status == CardStatus.IMPLEMENTING and not card.implementer:
            card.implementer = fallback
            changed = True
        elif card.status == CardStatus.REVIEWING and not card.reviewer:
            card.reviewer = fallback
            changed = True
        ...
        if changed:
            ...
            await self.broadcast(session_id, {"type": "card_updated", "card": card.to_dict()})
            log.info(
                "card %s missing assignee for %s; assigned fallback %s",
                card.id, card.status.value, fallback,
            )
            return fallback
        return None
