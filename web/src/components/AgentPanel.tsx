                  ...
                  <div key={msg.id} className="text-xs">
                    {msg.role.startsWith("dm:") ? (
                      <div className="flex items-center gap-1.5 text-[10px] text-violet-400/70 py-1">
                        ...
                        {msgTime && <span className="text-violet-300/40 ml-auto">{msgTime}</span>}
                      </div>
                    ) : msg.interrupted ? (
                      <div className="opacity-40">
                        {(msg.round_number || msgTime) && (
                          <div className="text-[10px] text-zinc-600 mb-1 flex items-center gap-2">
                            {msg.round_number && <span>Round {msg.round_number}</span>}
                            {msgTime && <span className="text-zinc-700">{msgTime}</span>}
                          </div>
                        )}
